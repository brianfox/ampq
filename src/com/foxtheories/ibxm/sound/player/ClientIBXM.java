...
public class ClientIBXM extends Thread {
	...
	ClientNote blankNote;

	private ArrayBlockingQueue<ClientNote> noteQueue;
	...
	public ClientIBXM( int sample_rate, Module m ) {
		if( sample_rate < 8000 ) {
			sample_rate = 8000;
		}
		...
		noteQueue = new ArrayBlockingQueue<ClientNote>(1);
		
		int channel_idx;
		module = m;
		blankNote = new ClientNote(module.get_num_channels(), module.get_pattern(0).num_rows);
	...
	public void addBlankNote() throws InterruptedException {
		noteQueue.put(blankNote);
	}
	...
	public int calculate_song_duration() throws InterruptedException {
		int song_duration;
		set_sequence_index( 0, 0 );
		next_tick();
	...
	public void seek( int sample_position ) throws InterruptedException {
		set_sequence_index( 0, 0 );
		next_tick();
	...
	public void get_audio( byte[] output_buffer, int frames ) throws InterruptedException {
	...
	private boolean next_tick() throws InterruptedException {
		int channel_idx;
		boolean song_end;
		for( channel_idx = 0; channel_idx < channels.length; channel_idx++ ) {
	...
	private boolean next_row() throws InterruptedException {
	...
	public void run() {
		try {
			...
			while( open ) {
				int frames = 1024;		
				get_audio( output_buffer, frames );
